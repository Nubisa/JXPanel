...
$.SmartMessageBox = function(a, b) {
    ...
    window.botClick = function(_id) {
        window.botClick = null;
        var a;
        if(!_id)
            a = $(this).attr("id");
        else
    ...
    var g = a.beforeInput ? a.beforeInput : "",
        attr = a.inputAttr ? a.inputAttr : "",
        h = 0,
        inputId = a.inputId ? a.inputId : SmartMSGboxCount;
    if (void 0 != a.input) switch (h = 1, a.input = a.input.toLowerCase(), a.input) {
        case "text":
            a.inputValue = "string" === $.type(a.inputValue) ? a.inputValue.replace(/'/g, "&#x27;") : a.inputValue
                , g += "<input class='form-control' type='" + a.input + "' id='txt" + inputId + "' placeholder='" + a.placeholder + "' value='" + a.inputValue + "'" + attr + "/><br/><br/>";
            break;
        case "password":
            g += "<input class='form-control' type='" + a.input + "' id='txt" + inputId + "' placeholder='" + a.placeholder + "'" + attr + "/><br/><br/>";
            break;
        case "select":
            if (void 0 == a.options) alert("For this type of input, the options parameter is required.");
            else {
                g += "<select class='form-control' id='txt" + inputId + "'" + attr + ">";
    ...
    d = "<div class='MessageBoxContainer animated fadeIn fast' id='Msg" + inputId + "'>", d += "<div class='MessageBoxMiddle'>", d += "<span class='MsgTitle'>" + a.title + "</span class='MsgTitle'>", d += "<p class='pText'>" + a.content + "</p>", d += g, d += "<div class='MessageBoxButtonSection'>", void 0 == a.buttons && (a.buttons = "[Accept]"), a.buttons = $.trim(a.buttons), a.buttons = a.buttons.split("");
    var j = "",
        k = 0;
    ...
    for (var i = 0; i <= a.buttons.length - 1; i++){
        if("[" == a.buttons[i])
            ...
            k += 1;
            window.activeBot = "bot" + parseInt(k) + "-Msg" + inputId;

            j = "<button id='bot" + k + "-Msg" + inputId + "' class='btn btn-default btn-sm botTempo'> " + j + "</button>";
            d += j;
        }else{
            j += a.buttons[i];
        }
    }
    d += "</div>", d += "</div>", d += "</div>", SmartMSGboxCount >= 1 && ($(".MessageBoxContainer").hide(), $(".MessageBoxContainer").css("z-index", 99999)), $(".divMessageBox").append(d), 1 == h && $("#txt" + inputId).focus(),
    $(".botTempo").hover(function() {
        $(this).attr("id")
    }, function() {
        $(this).attr("id")
    }),
    $(".botTempo").click(function(){botClick($(this).attr("id"))});
};
window.onkeydown = function(ev){
    if(window.botClick){
        ev = ev || window.event;
        var key = ev.keyCode || ev.keypress;
        if(key == 13){
            botClick(window.activeBot);
        }
    }
}
var BigBoxes = 0;
